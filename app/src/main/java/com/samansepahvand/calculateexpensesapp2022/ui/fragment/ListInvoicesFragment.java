...
public class ListInvoicesFragment extends Fragment implements SearchView.OnQueryTextListener, View.OnClickListener, ListInvoicesAdapter.IGetMetaInfo {
    ...
    private OperationResult<InfoMetaModel> result;
    ...
    private void intiData() {
        ...
        result = InfoRepository.getInstance().GetInfo(dateModel);
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        if (result != null) {
            for (InfoMetaModel item : result.Items) {
                if (item.getTitle().toLowerCase().contains(newText) ||
                        item.getFarsiDate().toLowerCase().contains(newText) ||
                        item.getPriceTypeItemName().toLowerCase().contains(newText)) {
                    newList.add(item);
                }
    ...
    public void GetMetaInfo(InfoMetaModel metaModel, int invoiceActionType) {
        ...
        if (invoiceActionType == Enumerations.InvoiceActionType.DELETE) {
            result = InfoRepository.getInstance().GetInfo(dateModel);
            txtInvoiceCount.setText("تعداد : " + (result.Items.size()));
            txtTotalPrice.setText(Utility.SplitDigits(Integer.parseInt(result.Message)));
        } else {
